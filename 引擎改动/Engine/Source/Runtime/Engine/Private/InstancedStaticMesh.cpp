...
void UInstancedStaticMeshComponent::BuildRenderData(FStaticMeshInstanceData& OutData, TArray<TRefCountPtr<HHitProxy>>& OutHitProxies)
{
	...
	for (int32 Index = 0; Index < NumInstances; ++Index)
	{
		...
		//++[NEXT] [MRD] [sn] calculate instance id by sequence
		float RandomInstanceID = RandomStream.GetFraction();
		if (OverrideInstanceRandom)
			RandomInstanceID = (float)Index / (float)NumInstances;
		//--[NEXT] [MRD] [sn]
		OutData.SetInstance(RenderIndex, InstanceData.Transform, RandomInstanceID, LightmapUVBias, ShadowmapUVBias);
