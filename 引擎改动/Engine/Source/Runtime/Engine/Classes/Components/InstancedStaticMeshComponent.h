...
class ENGINE_API UInstancedStaticMeshComponent : public UStaticMeshComponent
{
	...
	/**++[NEXT] [MRD] [sn] INSTANCING : whether to override the random with instance index. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = InstancedStaticMeshComponent)
		bool OverrideInstanceRandom;
	...
	*/
	...
	//++[NEXT] [MRD] [sn]  for culling

	/**Upload dynamic Instanced data*/
	void UploadInstanceData(int StartIndex, int EndIndex);
	/**INSTANCING PATCH: whether to override the instances number. negative use original */
	void SetNumVisibleInstances(uint32);
