#include "InstanceGPUSkinAnimActor.h"
...
void FUCrowsInstDataGpuSkin::Tick(float DeltaTime)
{
	AnimTimeCounter += DeltaTime * AnimPlayRate;
	FTableRowGpuSkinAnimData* rowInfo = &ParentActor->AnimSeqDatas[AnimSeq];
...
FTransform AInstanceGPUSkinAnimActor::GetInstTransform(int id)
{
...
FUCrowsInstDataGpuSkin AInstanceGPUSkinAnimActor::GetInst(int id)
{
	if (id >= 0 && id < InstLst.Num())
	{
		return InstLst[id];
	}
	return FUCrowsInstDataGpuSkin();
}

AInstanceGPUSkinAnimActor::AInstanceGPUSkinAnimActor()
{
...
void AInstanceGPUSkinAnimActor::BeginPlay()
{
	Super::BeginPlay();

	UE_LOG(LogTemp, Log, TEXT("LOGDEBUG - AInstanceGPUSkinAnimActor [%s] do BeginPlay()"), *this->GetName());
	...
	if (AnimDataTable)
	{
		...
		for (int i = 0; i < RowNames.Num(); ++i)
		{
			...
			FTableRowGpuSkinAnimData* pRow = AnimDataTable->FindRow<FTableRowGpuSkinAnimData>(name, ContextString);
			if (pRow)
			{
	...
	{
		FUCrowsInstDataGpuSkin inst;
		inst.AnimSeq = UKismetMathLibrary::RandomIntegerInRange(StartAnimIndex, EndAnimIndex);
		if (MakeAllInstPlayAnimID >= 0)
			inst.AnimSeq = MakeAllInstPlayAnimID;
		FTableRowGpuSkinAnimData& animData = AnimSeqDatas[inst.AnimSeq];
...
void AInstanceGPUSkinAnimActor::Tick(float DeltaTime)
{
	...
	for (int i = 0; i < DynamicMaterialInstances.Num(); ++i)
	{
		...
		DynamicMaterialInstance->SetTextureParameterValue(VertBlendInfoTexParamName, TexVertBlendInfo);
		DynamicMaterialInstance->SetScalarParameterValue(VertBlendTexWidthParamName, TexVertBlendInfo->GetSizeX());
		DynamicMaterialInstance->SetScalarParameterValue(VertBlendTexHeightParamName, TexVertBlendInfo->GetSizeY());
	}
	...
	{
		FUCrowsInstDataGpuSkin* inst = &InstLst[idx];
		inst->Tick(DeltaTime);
...
void AInstanceGPUSkinAnimActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
